...
A candy-goth endless runner for the portfolio shell. **Nix**, the
wish-thief kitten, descends through four moonlit districts of a sleeping
night-city — Rooftops, Lantern Market, Frostglass Gardens, The Undernight —
while you jump (twice for a double), dash through danger, and keep a
three-heart meter alive. Hearts and big cross-hearts mend her; crates break
her. Chain pickups into a combo multiplier for score — every hazard in the
run can be cleared with a well-timed jump.

Nix is an original character — a pocket-sized dusk-kitten in a wisp-hood,
wide-eyed and up to something. She grins (one tiny fang, only when she's
delighted) because the wishes she collects are never coming back — but she
will keep them safe forever, which is the kindest theft there is. She is a
deliberate homage to the gothic-cute archetype: flat vector shapes, one
crisp ink outline, a single spectral-mint accent — with a fully distinct
silhouette (a chipped ear, big dual-catchlight eyes, a wisp-hood that
streams into forked tails, and an upward wisp-light tail) and an original
name, so the archetype reads while every existing mascot's trade dress
stays someone else's.

Four features carry the engineering story:

- **Night Districts** — the run is a journey, not a lap. Four districts,
  each its own palette lerp, scenery mood and hazard tint, keyed purely to
  distance so the seams always arrive on schedule. The sky crossfades
  between four baked gradient variants, hills/clouds/crates carry a
  ratio-multiply tint (district I is byte-identical to the base palette),
  and the moon rides its own aspect-correct plane so it stays round on
  every viewport.
- **Difficulty director** — the pattern mix's ramp is no longer a fixed
  650 m curve: a skill snapshot (stored with your best-run replay) decides
  how fast the night bites, from a gentle 900 m span for novices to a mean
  480 m for veterans. The ratchet is deterministic and past-only: a deep,
  healthy run raises the stored skill (capped at ±0.1 per best); a weak
  run grants gentle relief. Because the track is a pure function of
  (seed, skill, distance), the ghost always races you on the identical
  track.
...
- **Adaptive soundtrack** — no audio files. A Web Audio lookahead
  ...
  sound effects.
- **Race your best-run echo** — a finished run is stored as seed, skill
  snapshot plus timed inputs; a faded afterimage replays it on the very
  same track and gives chase once you open a lead. Same seed, same
  skill, same physics, same bullet-time — a deterministic replay you can
  outrun. Near-misses get their own beat: graze a hazard and Nix's eyes
  pop, her ears prick, a mint sparkle sings.
...
- **Race your best-run echo** — a finished run is stored as its seed, its
  skill snapshot plus the timed input list; the next runs reuse both, so a
  faded spectral afterimage of Nix replays your finest hour on the very
  same track.
...
- **Zero image assets** — the sky, moon, clouds, hills, crate dots and the
  particle sprite are canvas-generated textures; Nix herself is procedural
  vector art: `THREE.ShapeGeometry` parts (round head, tall ears — one
  chipped, wisp-hood with forked tails, romper, wisp tail, big
  dual-catchlight eyes) with an ink copy grown behind each fill as an
  outline. She is drawn in code exactly so the character stays hand-made
  and the silhouette is ours.
- **Responsive framing** — `web/lib/framing.ts` derives camera distance,
  ...
  eleven world units of run-ahead on portrait phones (Nix reads as a
  ...
  stage.
- **Depth discipline** — the camera near plane sits at 2 and Nix's
...
- **Contact shadow** — a soft canvas-texture ellipse rides `groundY()`
  under Nix, tightening and fading as she climbs, so she lands *on*
...
- **First impression** — the ready overlay is a low plum wash, and its
   wish-card parks in the run-ahead space, so visitors see Nix idling in
