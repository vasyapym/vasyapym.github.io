...
## Parked — feature reverted (owner verdict, live-browser round)
- Owner verdict after the first real-browser round: all four fates rendered black (the past timeline intact); the on-site impression was "scope creep, not worth it now". Decision: revert the fates code, keep the design work, return later.
- Diagnosis context gathered before the verdict (kept in the record): the black frames were real — a feedback loop (Playwright Chromium at ~/Library/Caches/ms-playwright/chromium-1134, drive via CHROME_PATH) showed the fate draw shader compiled only after a missing-uniform splice fix, then still rendered empty (heatDeath) or a single blown-out blob (bigRip). Root causes identified but NOT fixed: scale mismatch (engine box=1 comoving vs the past field's radius-40 world and a camera parked at ~76) and an unbounded point-size clamp merging 220k additive sprites. The uCamC/uProj/uView uniform declarations had also been spliced out of the shader text during C004 integration (caught by the loop, fixed in the worktree, never shipped).
- Revert scope: deleted src/fates.ts, src/fatesParticles.ts, src/fatesUi.ts, tests/fates.check.ts, tests/fatesParticles.check.ts; restored package.json test chain and src/main.ts to the pre-B1 state (3936301). KEPT: docs/fates-physics-design.md, all briefs/prompts in docs/briefs/, this pass record, the project-graph nodes — the roadmap for the return.
- Return checklist (for the future session): (1) fix scale coherence first — either box=80 or a fate-mode camera refit; (2) cap additive sprite size; (3) keep the fates-smoke.mjs approach (brightness probe + per-fate deep links) as the red-capable loop — it caught every defect above; (4) B5 (vacuum bubble wall) and the design round remain open.
- Verification of the reverted state: npm run verify PASS (cosmology.check only, original chain); browser smoke PASS 12/12 (gpgpu 42fps, scrub, poke, static fallback, mobile) — the site is back to its pre-fates behavior.
