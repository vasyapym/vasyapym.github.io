...
## Round R035
- Goal: owner's batch — (1) a project entered via /about must return there, not to the main page; (2) the return-restore scroll must glide, not snap; (3) ¶ marks on mobile too; (4) project names prominent in the gray mono voice of the previous iteration; (5) vasyapym.github.io and "the deep" highlighted the same way; (6) ¶3 reworded ("What holds me is watching how they rebuild a picture of the world, out of nothing but the texts read"); (7) the lead's lantern sentence becomes its own paragraph; (8) the measure centered; (9) body +15% (14→16px).
- Preserved preferences: F002 (corridor), F005, F006–F013, R032 colophon, R033 copy order, R034 fixes.
- Changes: `project-return-intent.ts` — the intent now carries the origin path (default "/"); new `animated-scroll.ts` — house easeInOutCubic glide, distance-scaled 480–1000ms, wheel/touch cancel, reduced-motion = instant; `App.tsx` — openProject/openAbout remember the current pathname, goHome branches to /about (tagged entry, intent left live for the page's own restore), renderAboutLink gains the /deep pseudo-route (clears stale project intent, seeds the r11 realm intent, boots the landing with the realm open); `LandingPage.tsx` — the restore glides via animateScrollToY; `AboutPage.tsx` — lead becomes Segment[][] with ghost marks (hidden on mobile), unnumbered-link segment variant, mount effect consumes the /about intent + glides; `about-copy.ts` — the reworded ¶3, lead split, vasyapym.github.io → "/" link, the deep → "/deep" link; `about-page.css` — 16px body, ¶ marks on mobile (block above each paragraph, ghost cells hidden), centered measure (1fr/62ch/1fr with the rail right-aligned into the measure), .ab-link = gray mono 0.9em waking to paper on hover/focus.
- Before: artifacts/R034/about-top.png
- After: artifacts/R035/about-1440x900.png, about-390x844.png, deep-1440x900.png
