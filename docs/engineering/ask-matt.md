...
People keep asking for one in the README. This skill is that list — it is what it exists for. A static table would say `grill-with-docs → design-planning → planning → to-spec → to-tickets → implement → code-review` and be wrong for most situations, because the interesting parts are the branches — is there a codebase, does the build span sessions, can this question be settled by talking, and does the design need a prototype. An existing visual feedback round is a separate reach for `/design-iteration`, which keeps an append-only liked/rejected ledger instead of reopening the whole flow. A non-trivial code change can likewise start at `/code-iteration` when it needs evidence-backed verification passes rather than one-shot edits. Where a project has a project graph, those two also exchange typed handoff edges instead of relying on whoever remembers last session's notes. Technology learning itself belongs at `/custom-learning`, which keeps one Subcard active and gates progress on a proof artifact. The honest cost is that the router is hand-maintained and lags the repo. `/grilling` and `/resolving-merge-conflicts` both shipped long before the router named them.
