# BRIEF — Compaction round 4: portfolio-redesign handoff doc, part 1 of 2 (policy locked)
...
The repo drives a multi-agent workflow. This document (`docs/portfolio-redesign-handoff.md`) is the durable design record of the finished main-page redesign: the active "Ink catalogue" design system, three superseded design directions, a compact decision graph, and an append-only iteration ledger (one prose section per design pass). Project-graph event logs reference it by URL anchors built from its headings — **anchors are frozen**. The raw pre-compaction text survives in git history, so aggressive prose compression is safe.

Part 1 covers: the title/intro, the active Ink-catalogue handoff, the three superseded handoffs (Refraction sea, Dark catalogue, Quiet index), the compact decision graph, and the iteration ledger's early passes (through Pass 22). Part 2 (separate brief) covers Pass 23 onward plus the handoff protocol.

## Hard rules

1. **Every heading line must be reproduced byte-identically, same order, nothing added.** (The `#`, `##`, `###` lines — including em-dashes, dates, and trailing punctuation. They ARE the anchors.)
2. Under each heading, compact the content:
   - **Active handoff (Ink catalogue)**: keep the load-bearing spec — tokens, palette laws, type laws, layout structure, quality gate — as tight bullets/lines (aim ~15-25 lines total). Drop code snippet bodies; keep one-line statements of what each establishes.
   - **Superseded handoffs**: 2-4 lines each: the direction, why it lost, what carried into the winner.
   - **Compact decision graph**: keep as-is if already dense; otherwise tighten. Do not renumber anything.
   - **Iteration ledger passes**: per pass, 1-3 dense lines: what it changed (one clause), the owner verdict (adopted/rejected/pending — both states if the trail matters), the lesson/gate if one was born there. Drop step narration, file lists, viewport enumerations, screenshot references, and repeated "verified typecheck+build" boilerplate (keep a verdict-level "gates green" where meaningful).
   - Owner quotes: keep only when the quote itself is the decision, in one clause.
3. Do not invent facts. Do not lose a verdict, a standing constraint, or an open thread. When two adjacent passes repeat the same context, land it in the earlier one; the later keeps its delta.
4. Preserve the persistent-decisions content wherever it appears in this part (it is the survive-every-direction-change list — tighten wording, keep every decision).

## Output contract

- Output = the compacted **markdown for this part only**, starting with the part's first line (the `# Portfolio redesign — design handoff` title) and ending at the part's last line.
- No preamble, no commentary, no code fences around the whole output. If you approach output limits, end with the line `TRUNCATED — reply CONTINUE` and resume exactly where you stopped after I reply.
- Every heading from the input part must appear in your output in the same order (a verifier checks this; a missing or altered heading breaks graph anchors).

## Input (raw part 1, verbatim)
