# BRIEF — Evening Forest: adaptive score pass (`web/lib/music.ts`)

You are revising one complete TypeScript module for a shipped web game. You
...
(Proteus-inspired). The file below is its procedural ambient score: a
breathing D-aeolian drone, slow open-fifth pad swells, a sparse bell-like
melody, a rare distant chime and a filtered night-air bed, glued by a bus
compressor and a runtime-built convolution reverb, scheduled as a seamless
38.4 s cycle. It already works and sounds good — **this is a refinement
pass, not a rewrite**: keep every voice, the signal graph, the teardown
semantics and the code style exactly as they are.

What's missing: the score ignores the world. In Proteus, the music responds
to time of day and movement. Your task: make the score **gently adaptive**
using the environment bridge specced below.
...
// lazily (inside scheduling callbacks), never snapshot it at setup.
export const audioEnv = {
  timeOfDay: 0,   // 0..1 — 0 golden hour, 0.55 deep night, 1 sunrise
...
Define these small private helpers locally in the file (do NOT import them
from sibling modules):

```ts
// 0..1, triangular: 0 for t <= 0.15, 1 at t = 0.55, 0 again by t >= 0.95.
function nightness(t: number): number
function clamp01(v: number): number
```

## Required adaptive behaviour (all subtle — night stays calm, nothing jumps)

Read `audioEnv` at **schedule time** (inside `scheduleCycle` and per-note),
so the 38.4 s cycle inertia makes every change slow and smooth. The one
continuous exception is the drone/air level update below.

1. **Melody register follows the arc.** Note choice is currently
   `floor(rng() * MELODY.length)`. Replace with a "gravity" pick: a centre
   index `3.0 - nightness * 1.8` plus spread `(rng() * 2 - 1) * 2.2`,
   rounded and clamped to the scale — so night lines sink toward the low
   D3/F3 region, golden hour and sunrise float mid-register.
2. **Night is sparser and softer.** The rest probability becomes
   `0.15 + nightness * 0.15`; each note's spacing multiplier becomes
   `1 + nightness * 0.25`; note level scales by `1 - nightness * 0.2`.
   Walking tightens phrasing slightly: multiply spacing by
   `1 - clamp01(audioEnv.moveSpeed / 3.4) * 0.15`.
3. **Darker tone at night.** The melody lowpass `1800` becomes
   `1800 - nightness * 700`; the pad lowpass `1200` becomes
   `1200 - nightness * 250`; pad level scales by `1 - nightness * 0.25`.
4. **Darker chords at night.** Pad chord choice: when
   `nightness > 0.5`, pick from the two darker voicings only
   (`D-G-A` sus4, `D-A-C` min7); otherwise from the full list.
5. **Chime: rarer at night, more likely near the fox.** Cycle probability
   `0.5` becomes `0.5 * (1 - nightness * 0.5) + (audioEnv.foxDist !== null &&
   audioEnv.foxDist < 12 ? 0.3 : 0)`. At night prefer the lower tone (D5).
6. **Continuous drift on the 1 s scheduler.** The existing lookahead
   interval ALSO applies, once per tick, `setTargetAtTime` (timeConstant 2):
   - `droneGain.gain` target `DRONE_LEVEL * (1 - nightness * 0.18)`
     (safe to schedule: the breath LFO adds on top of the param value),
   - `airGain.gain` target `AIR_LEVEL * (1 + nightness * 0.2)`.
   Keep this on the SAME interval callback that commits cycles; do not add
   a second timer.

Nothing else changes: no new voices, no level re-balancing beyond the
factors above, no signature changes.

## Hard constraints

- Imports: only `./rng` and `./audio-env`. No three.js, no react; the only
  window access stays the single `setInterval`. **No Math.random** — the
  seeded rng drives everything, exactly as now.
- Public API identical: `createEveningMusic(ctx, out): EveningMusicVoice`.
  Keep `MUSIC_LEVEL`, `CYCLE`, `SCHEDULE_AHEAD`, the compressor/makeup
  chain, `buildImpulse`, the drone + air setup, `register()`, `playNote`,
  `playPad`, `playChime` structures and the full teardown.
- Update the header comment (lines 1–6) to describe the adaptive score in
  the same calm voice.
- Style: 2-space indent, function declarations, explicit `: void`, no
  classes, no dead code, no emojis.
...
1. The complete replacement `music.ts` in one fenced ```ts block.
2. A short list (≤ 8 bullets): what you changed and anything the integrator
   should listen for. No restated spec, no extra prose.
