...
4. main page realm — r2 (n152–n154), r3 passes A–D (n156–n157, n159, n161), then the threshold visual rounds r4 (n163–…): the entry is an in-flow threshold section between hero and cards, now redesigned as the **conserved count** — "07 doors" in an index-like left margin (the noun flips works→doors on reveal; hovering/keyboard-focusing "enter the deep" flips it back and out again), one-line sentence "the same work, beneath the surface.", compact ~159px band; the fixed strip = the liked mobile presentation (188×44 bottom-right) on all widths under ONE reversible law — visible past the threshold section, hides again on scroll-back; macOS Safari "blackish sides" fixed by route-owned `data-signal-index` document backdrop; flood seeds from the activated control, exit focus restored. Rejected threshold directions (compressed): recessed descent+ochre sill ("good but boring"), five geometry-prop concepts (oblique/poster/soundings/cover/crossing), operative-sentence + refracted-word (r4-C pick was conserved count). Probe 39/39 (exit gates now poll the unmount — fixed waits fail under SwiftShader load). Owner device checklist pending: native Safari bands (incl. /art-directions nav), iOS surface-flash re-check.
