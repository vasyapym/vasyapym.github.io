...
4. main page realm — r2 (n152–n154) then r3 passes A–D (n156–n157, n159, n161): the entry moved into an in-flow threshold section between hero and cards ("another way through / the same work, beneath the surface.", hairline draw-in, ochre-intent button); the fixed strip = the liked mobile presentation (188×44 bottom-right) on all widths under ONE reversible law — visible past the threshold section, hides again on scroll-back; macOS Safari "blackish sides" fixed by route-owned `data-signal-index` document backdrop (the old `:root:has(.signal-index)` also matched /art-directions); flood seeds from the activated control, exit focus restored. Probe 39/39. Owner device checklist pending: native Safari bands (incl. /art-directions nav), iOS surface-flash re-check.
