...
4. main page realm — r2 feedback round shipped (n152–n154, brief 14), then r3 (n156–n157, brief 15): the entry chip redesigned to the owner-chosen "threshold bar" (bottom-left, from 5 relayed concepts) with a new visibility law — desktop visible from first render (the old band mathematically inverted at 2560×1440 and never showed the chip; owner's Edge report), mobile one-shot reveal; iOS bugs fixed: surface white flash = light document backdrop exposed at unmount retile (declarative `:root:has(.signal-index)` dark backdrop), mid-screen bottom sheet = the mobile layer now tracks `visualViewport` into `--realm-visible-*` + panel capped at 48%. Probe 31/31. Owner real-device checklist pending: surface-flash absence on iPhone (URL-bar states), sheet bottom anchoring, 2560×1440 chip visibility, lantern/wake feel numbers from brief 14.
